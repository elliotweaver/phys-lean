...
theorem braVal_eq : braVal = -2 := by
  have h : ⁅innerMul (slotA (1:O ℚ)) (slotB 1), Lmul (slotA 1)⁆
      (1 : Matrix (Fin 3) (Fin 3) (O ℚ)) = - slotB 1 + - slotB 1 := by
    rw [family_moves_mover Matrix.conjTranspose_one, Lmul_one]
  unfold braVal
  rw [h, Matrix.add_apply, Matrix.neg_apply, slotB_entry]
...
theorem bra_scale_wrong_BOGUS :
    min (325 : ℕ) braVal.num.natAbs = 325 := by
  rw [bra_scale_true]
  -- ⊢ (2 : ℕ) = 325  (FALSE — the costume bites)
