/-
  ...
  sharpened restatement of the ATTESTED-COMPLETE recomposed census-conserving endpoint AS `1/α(0)`) is
  GENUINE: the re-assembled bracket is strictly two-sided (`1363071881405/1e10 < 1363071881406/1e10`),
  the endpoint lies strictly below `137` (the fold's prediction, a real exclusion of the measured value),
  and rung 2 genuinely improves rung 1's heavy-shape-log bracket (`17337287/1e7 < 1733728794954/1e12` and
  `1733728794955/1e12 < 17337288/1e7`). C571.
  =====================================================================================
  W8 ANTI-VACUITY. This node (THE D7 NARROWING RUNG 2) pushes the three edge logs to `≈ 10⁻¹²` and
  re-assembles a ≈ 12-significant-digit certified two-sided rational bracket of the ATTESTED-COMPLETE
  (ledger-CLOSED) recomposed census-conserving endpoint. The content that must NOT be hollow is that the
  re-assembled bracket is a GENUINE two-sided reading (`restated_r2_strict`, `1363071881405/1e10 <
  1363071881406/1e10`), that the endpoint is GENUINELY below `137` (`recomposedEndpoint_lt_137_r2` — a
  real exclusion of the measured `≈ 137.036`, not a vacuous inequality), and that rung 2 GENUINELY
  tightens rung 1's edge bracket (`rung2_improves_rung1`, `17337287/1e7 < 1733728794954/1e12` and
  `1733728794955/1e12 < 17337288/1e7`). If the re-assembled bracket were collapsed, or the endpoint were
  not below `137`, or the rung did not improve rung 1's `[17337287/1e7, 17337288/1e7]`, the rung would be
  vacuous. It is genuine.
  ...
  rung does not improve rung 1 / the rung is vacuous" reading): that min 571 attestFlag = 571. It
  GENUINELY equals 1 (cert_val_true). Rewriting reduces the bogus claim to the false numeric 1 = 571 in
...
/-- BOGUS: claims min 571 attestFlag = 571. It GENUINELY equals 1 (cert_val_true). A "the bracket is
    ...
    rung2_improves_rung1 tie -- to the false numeric 1 = 571 in the naturals. The kernel cannot close it;
    it BITES. -/
