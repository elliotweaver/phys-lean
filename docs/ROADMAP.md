...
FINDINGS.md for the full measured argument.
  ...
  - **★ (FORWARD FRONTIER — the immediate next node, N51) THE FULL SURJECTIVITY onto the connected component
    `SO⁺(1,9)`, childed from N50.** N49 + N50 banked BOTH one-parameter generator families over the derived ℝ
    `Cut`: the hyperbolic BOOST `boostC_isom` (the non-compact, time-mixing generator) and the compact ROTATION
    `rotC_isom` (the space-rotating generator), both members of the continuum isometry monoid `qvIsomMonoidC ⊆
    Function.End STVC`. THE TARGET: that EVERY orientation/time-preserving continuum isometry of `QvC` is a
    finite product of these generators — i.e. `qvIsomMonoidC` restricted to the connected component IS generated
    by the one-parameter subgroups. ⚠ THIS IS THE GENUINELY HEAVY core: a deep Cartan/polar-decomposition /
    generation-by-one-parameter-subgroups theorem; Mathlib has NO `SO⁺(1,9)`, no octonionic spinor cover, no Lie
    machinery for it — it must be BUILT over the derived ℝ + the banked Aut(𝕆)/exp stack (N33–N41g), NOT Mathlib
    ℝ/ℂ as content, NOT a posited Lorentz group. W1/W3: MEASURE FIRST (the smallest obligation — e.g. that a
    single one-parameter subgroup over `Cut`, via the banked exp stack, lands in `qvIsomMonoidC` and sweeps a
    continuous family; or that the boost+rotation generators close into the connected component's tangent
    algebra) before committing. Reframe through the trunk FIRST: a classical Lie-surjectivity grind may DISSOLVE
    when the connected component is attacked through the banked `Der(𝕆)=g₂` / Aut(𝕆)/exp correspondence (the
    derivations ARE the infinitesimal generators — a trunk reframe that may dissolve the coordinate
    polar-decomposition grind) rather than by a coordinate polar decomposition. If the full surjectivity resists
    after measure + reframe through the derived-ℝ exp stack and the derivation/automorphism correspondence, it is
    an HONEST W1 dissolution ticket + block, NOT a grind. The costume must bite a WRONG generation/
    covering-degree claim. NO posited Lorentz group, NO Mathlib ℝ/ℂ as content — DERIVE from the banked
    continuum generators + the derived ℝ.
