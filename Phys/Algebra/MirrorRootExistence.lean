...
theorem root_interior {a : Cut} (ha : a ∈ Icc aL aR) (hroot : residual a = 0) :
  ...
  · rcases eq_or_lt_of_le ha.1 with h | h
    · exfalso
      have : residual aL = 0 := by rw [h]; exact hroot
      exact absurd this (ne_of_lt residual_neg_at_aL)
    · exact h
  · rcases eq_or_lt_of_le ha.2 with h | h
    · exfalso
      have : residual aR = 0 := by rw [← h]; exact hroot
      exact absurd this (ne_of_gt residual_pos_at_aR)
    · exact h
