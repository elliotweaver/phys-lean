/-
  Phys.Algebra.DerivationStructureConstants — N21 (ii): THE g₂ LIE STRUCTURE of
  Der(O ℚ), PART 2 — the explicit Lie-bracket STRUCTURE CONSTANTS.
  ===========================================================================
  N21 part 1 (`DerivationBracket.lean`) banked the commutator closure over ℚ
  (`isDerivQ_bracket`), the 14-dimensional Lie algebra `derivationLieQ`, and the
  explicit 14-element basis `derivBasis` (the banked derivations `D0E..D13E`).

  THIS MODULE banks the FULL STRUCTURE-CONSTANT TABLE: for every pair `i < j` of the
  14 basis derivations, the commutator `⁅DiE, DjE⁆` is computed explicitly as a
  ℚ-combination `Σ cᵏᵢⱼ DkE` of the basis and PROVED forward. Because each `⁅DiE,DjE⁆`
  is again a derivation (`isDerivQ_bracket`), it lies in the 14-dim space `derivationQ`,
  so it MUST be such a combination; the structure constants `cᵏᵢⱼ` are read off and the
  identity is proved by coordinate extensionality (`ext` + `simp` on the derived
  Cayley–Dickson product + `ring`). All 91 pairs are banked; the structure constants are
  INTEGERS in `{-2,-1,1,2}` (84 of the 91 pairs are nonzero). This IS the `g₂` Lie
  bracket table, derived end-to-end from the fold's cascade.

  ★ THE TRUNK REFRAME (THE ONE LAW). The bracket is `⁅DiE,DjE⁆ = DiE∘DjE − DjE∘DiE`,
  a composition of the explicit sparse coordinate maps — NOT a monolithic table-bash.
  Each identity is its own bounded `ext`+`simp`+`ring` obligation (the W9.3/W9.4
  structure-over-expansion discipline: per-bracket lemmas, each cheap, never one
  14×14 normalizer). The single genuinely-octonionic entries are the two-term brackets
  (e.g. `⁅D4E,D7E⁆ = 2 D0E + 2 D3E`), where the non-associativity that STOPPED the
  cascade shows up as the associator contribution.

  THE MOAT — posit-vs-derive (docs/STANDARD.md §0). The division-algebra field POSITS
  `g₂` with a Cartan–Weyl bracket table by hand. Here every structure constant is
  COMPUTED from the commutator of the explicit derivations of the DERIVED octonion
  product and PROVED — no posited `LieAlgebra.g2` / `G₂`, no bracket table read off an
  exceptional Lie algebra. The `≅ g₂` Lie isomorphism (matching THIS proved table to a
  comparison `g₂`) is the dedicated downstream child N22 — never asserted here, never a
  bridge (Mathlib's exceptional-Lie support is thin; per W1 the comparison object is to
  be BUILT, not posited).

  PHYSICS-WORDS-REMOVABLE (docs/STANDARD.md §2). Delete "gauge / g₂ / automorphism":
  what remains is the pure statement that the commutators of the 14 explicit
  Leibniz-derivations of the Cayley–Dickson double of a double of a double of ℚ close,
  with the exhibited explicit integer structure-constant table. No theorem STATEMENT
