/-
  N169 — THE BOOST PEEL (the eigenframe-extraction core: signature (1,9) forces ONE boost).

  ── THE CHAIN POSITION ──
  N166 dissolved the N165 obstruction (the polar positive part is a positive self-adjoint
  `QvC`-isometry); N167 banked the RECIPROCAL CHARACTERIZATION `p ∘ J ∘ p = J` / `J p J = p⁻¹`;
  N168 banked the GENERAL-AXIS BOOST FACTOR `genBoostLin wx wv a b` — a `QvC`-isometry and the
  NON-VACUOUS `JoinedIdQvC` for a unit spatial direction `ŵ = (wx, wv)` and a forward hyperbola
  point `a² − b² = 1`, `0 < a`. The single residual closing the UNCONDITIONAL full `SO⁺(1,9)`
  identity component is `JoinedIdQvC p` for the positive self-adjoint `QvC`-isometry `p`.

  ── THE TRUNK REFRAME (THE ONE LAW, W1/W5) ──
  The eigenframe extraction does NOT need a standard spectral-theorem grind. The signature-(1,9)
  geometry IS the answer: `QvC (t,x,v) = t² − x² − gFormC v v` has EXACTLY ONE timelike direction,
  so any orthochronous `QvC`-isometry `p` mixes the timelike axis `e₀ = (1,0,0)` with AT MOST ONE
  spatial unit direction. That direction and the rapidity are READ DIRECTLY OFF `p e₀`:
    `p e₀ = (a, sx, sv)`, and `QvC (p e₀) = QvC e₀ = 1` gives `a² − (sx² + gFormC sv sv) = 1`;
    orthochrony `0 ≤ a` then forces `a ≥ 1`. Set `nrm := sx² + gFormC sv sv`, `b := cutSqrt nrm`,
    and the UNIT direction `ŵ := (sx/b, (1/b)•sv)` (degenerate `nrm = 0`: `p e₀ = e₀`, the identity
    boost). The FORWARD boost `B := genBoostLin ŵ a b` then satisfies `B e₀ = p e₀`, so the REVERSE
    boost `B⁻¹ = genBoostLin ŵ a (−b)` (N168's hyperbola, `a² − b² = 1`) sends `p e₀` back to `e₀`.

  ── WHAT THIS NODE BANKS ──
    `genBoostLin_comp_inv`              — the reverse boost `a(−b)` is the inverse of the forward
                                          boost `a b` along the same unit axis (the group structure
                                          of the boost family; an operator identity over the derived
                                          ℝ via `genBoostLin_apply` + `gFormC` bilinearity + an
                                          explicit `linear_combination` of `a²−b²=1` and the unit `ŵ`).
    ★★ `qvIsom_orthochronous_boost_peel` — THE BOOST PEEL: for an orthochronous `QvC`-isometry `p`
                                          (`0 ≤ (p e₀).1`) there is a unit direction `ŵ` and a
                                          forward hyperbola point `(a, b)` whose REVERSE boost sends
                                          `p e₀` back to `e₀`. The boost data is READ OFF `p e₀`.
    ★★ `qvIsom_orthochronous_boost_factor` — THE FACTORIZATION `p = B · k`: `B = genBoostLin ŵ a b`
                                          a forward general-axis boost, `k := B⁻¹·p` a TIME-FIXING
                                          `QvC`-isometry (`k e₀ = e₀`). ONE boost removes the unique
                                          timelike mixing, leaving a purely-spatial residual `k`.
    ★★ `joinedIdQvC_of_timeFixing_residual` — THE EIGENFRAME-EXTRACTION REDUCTION: if the
                                          time-fixing residual `k` is `JoinedIdQvC`, then `p` is the
                                          NON-VACUOUS `JoinedIdQvC` (the boost factor is `JoinedIdQvC`
                                          by N168's `joinedIdQvC_genBoostLin`, glued by N157's
                                          `joinedIdQvC_mul`). The boost factor is FULLY DISCHARGED;
                                          the full `SO⁺(1,9)` closure is reduced to the time-fixing
                                          `SO(9)` stabilizer (N163's territory).
    W8 `boost_peel_octReal_nonvac`      — TEETH: the peel of the concrete octonion-direction boost
                                          `(5/3, 4/3)` (which moves the octonion coordinate to
                                          `(4/3)•1` while leaving `x = 0`, N168) recovers boost data
                                          whose reverse boost returns `B₀ e₀` exactly to `e₀`. The
                                          peel genuinely identifies a non-trivial boost.

  ── W3 DECOMPOSITION — what remains (childed N170) ──
  The boost factor is now discharged UNCONDITIONALLY. The remaining residual closing the full
  `SO⁺(1,9)` identity component is: the TIME-FIXING residual `k` (`k e₀ = e₀`, a `QvC`-isometry) is
  `JoinedIdQvC`. This is exactly the `SO(9)` spatial stabilizer (N163 banked
  `joinedIdQvC_of_timeFixing_evCOrth_SO9` under EvC-orthogonality + det-`+1` hypotheses); the child
  certifies a time-fixing `QvC`-isometry meets those hypotheses (the purely-spatial closure), then
  `joinedIdQvC_of_timeFixing_residual` assembles the UNCONDITIONAL `JoinedIdQvC p`.
  ...
  isometry / self-adjoint / eigenframe / spectral / rapidity / hyperbola / timelike / spatial /
  orthochronous / time-fixing / stabilizer / path-connected / Joined): the statements stand as pure
  mathematics over the derived ℝ `Cut` and the terminal algebra `O Cut` — for a `Cut`-linear
  endomorphism `p` of `Cut × Cut × O Cut` preserving the form `QvC` with `0 ≤ (p (1,0,0)).1`, there
  exist `wx, wv, a, b, k` with `wx² + gFormC wv wv = 1`, `a² − b² = 1`, `0 < a`, `k` preserving
  `QvC`, `k (1,0,0) = (1,0,0)`, and `genBoostLin wx wv a b * k = p`; and IF `k` lies in the
  preconnected isometry set through `1` so does `p`. NO theorem STATEMENT needs a physics word.

  DERIVED from the banked chain: the general-axis boost factor `genBoostLin`/`genBoostLin_apply`/
  `genBoostLin_isQvIsomC`/`joinedIdQvC_genBoostLin` (N168); the form machinery `QvC`/`QformC`/
  `gFormC` + `gFormC_add_left`/`_smul_left`/`_smul_right`/`_zero_left`/`_one_left`/`_nonneg`/
  `_eq_zero_iff`/`reQC_one`; the square root `cutSqrt`/`cutSqrt_sq`/`cutSqrt_pos` (N57); the
  `JoinedIdQvC` engine `JoinedIdQvC`/`joinedIdQvC_mul` (N157). `LinearMap.ext`, `Module.End.mul_apply`,
  `linear_combination`, `field_simp`, `nlinarith` are MACHINERY on the DERIVED objects (STANDARD §3).
  ...
  posited KAK / polar product, NO posited boost flow / rapidity, NO posited eigenframe pairing, NO
  posited continuous power as content, NO regression to the vacuous `JoinedIdC`, NO Mathlib ℝ/ℂ as
  content, NO bridge.
-/
...
/-- ★ W8 NON-VACUITY / TEETH — the boost peel of the CONCRETE octonion-direction boost
    ...
    octonion-coordinate displacement is explicit. The peel genuinely identifies the boost —
    non-vacuous: `B₀ e₀ = (5/3, 0, (4/3)•1) ≠ e₀`, yet the reverse boost returns it exactly to `e₀`. -/
