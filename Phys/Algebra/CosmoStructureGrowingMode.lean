/-
  Phys.Algebra.CosmoStructureGrowingMode — N456 (cosmology-completion gap, directed successor of N455).
  ============================================================================================
  ...
  ── WHAT IS PROVED (each a theorem, foundations-only, forward from banked N174/N328/N359/N369/N383) ──
    ...
    growingMode_concrete_gt           — W8 tie for C481.
...
/-- ★★★ THE GRAVITATIONAL GROWING MODE / THE INSTABILITY OF THE SMOOTH STATE. For a positive initial
    ...
    times the Born density), the fold's own exponential flow `δ(t) = δ₀·cutExp(+ω·t)`:
      ...
      (4) is RECIPROCAL to the banked N383 decaying gap for EVERY `δ₁`: their product is
          time-independent, `= δ₀·δ₁` (`growing_decaying_product_conserved`) — the two C6 branches
          are one exponential (the genuine never-co-stated weld, statable from neither branch alone);
      (5) has FORCED rate `ω² = 4π·ρ` (`growthRate_sq`) — the banked 8πG Gauss-sphere flux times
          the Born density, NOT an independent constant.
...
theorem cosmo_structure_growing_mode (δ₀ ρ t₁ t₂ : Cut)
    ...
    (0 < growingMode δ₀ (growthRate ρ) t₁)
      ...
      ∧ (∀ δ₁ : Cut,
          growingMode δ₀ (growthRate ρ) t₁ * coherenceGap δ₁ (growthRate ρ) t₁ = δ₀ * δ₁)
      ∧ (growthRate ρ * growthRate ρ = growthRateSq ρ) := by
  refine ⟨growingMode_pos δ₀ (growthRate ρ) t₁ hδ,
          ...
          fun δ₁ => growing_decaying_product_conserved δ₀ δ₁ (growthRate ρ) t₁,
          growthRate_sq ρ hρ⟩

/-! ## §W8 — non-vacuity tie for C481. -/

/-- W8 TIE (for C481): a concrete growing mode GENUINELY exceeds its initial value —
