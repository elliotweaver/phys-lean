...
/-! ## ★★★ FULL IRREDUCIBILITY of the faithful 7-rep (FORWARD, NO posited G₂).

    `LieModule.IsIrreducible ℚ derivationLieQ ↥ImLie` unfolds to
    `IsSimpleOrder (LieSubmodule ℚ derivationLieQ ↥ImLie)`: the only Lie submodules of
    the 7-rep are `⊥` and `⊤`. A proper nonzero Lie submodule `N` would push forward to
    an invariant subspace `W ≤ ImO` with `0 < dim W < 7`, contradicting
    `no_proper_invariant_ImO`. This is the defining property of the type-G₂ fundamental
    7-representation, here a STRUCTURAL FACT about the banked `derivationLieQ`/`ImLie`. -/

section Irreducible

/-- The ambient image of a Lie submodule of `↥ImLie` is contained in `ImO`. -/
...
/-- The ambient image of a Lie submodule of `↥ImLie` has the same dimension
    (the inclusion `ImLie.incl` is injective). -/
...
/-- ★★★ FULL IRREDUCIBILITY of the faithful 7-rep: the only Lie submodules of `↥ImLie`
    are `⊥` and `⊤`. A proper nonzero submodule would yield a proper nonzero invariant
    subspace of `ImO`, excluded by `no_proper_invariant_ImO`. -/
