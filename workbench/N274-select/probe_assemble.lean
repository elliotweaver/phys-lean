...
-- Per-entry lemmas (each proven, from probe_entries). Assemble by Matrix.ext.
...
theorem e12 (a b : O ℚ) (d0 d1 d2 : ℚ) :
  ...
  rw [ocRmulR, ocRmulL, ocRmulR, ocRmulL, mul_add, add_mul, qsmul_mul_right, qsmul_mul_right,
    qsmul_mul_left, qsmul_mul_left, sub_smul]
  abel

-- assemble: whole-matrix from e12 + the zero cells closed by full simp
...
theorem whole (a b : O ℚ) (d0 d1 d2 : ℚ) :
  ...
  -- 9 goals; (1,2) is e12; the rest are zero = slotC..off-entry
  all_goals (try (rw [show slotC ((d2 - d1) • (star a * b)) = slotC ((d2 - d1) • (star a * b)) from rfl]))
  · -- (0,0)
    simp [slotC, Xz, jb, slotA, slotB, Dg, Matrix.mul_apply, Matrix.add_apply, Matrix.sub_apply,
      Fin.sum_univ_three, ocRmulL, ocRmulR]
  all_goals sorry
