...
-- STRUCTURAL simp ONLY (no distribution). Then close each entry with `first`.
-- The zeros should now be crushable by simp with zero_mul BEFORE any distribution.
set_option maxHeartbeats 1600000 in
theorem innerMul_slot_diag_B (a b : O ℚ) (d0 d1 d2 : ℚ) :
  ...
  fin_cases i <;> fin_cases j <;>
    ...
    first
      | rfl
      | (rw [ocRmulL, ocRmulR, mul_add, add_mul, qsmul_mul_left, qsmul_mul_right, sub_smul]; abel)
      | (simp only [ocRmulL, ocRmulR, mul_add, add_mul, qsmul_mul_left, qsmul_mul_right, sub_smul]; abel)
