...
-- ISOLATE the (0,0) zero entry. What structural output, what closes it?
set_option maxHeartbeats 800000 in
theorem zero00 (a b : O ℚ) (d0 d1 d2 : ℚ) :
    ((jb (slotA a) (jb (slotB b) (Dg d0 d1 d2))
      - jb (slotB b) (jb (slotA a) (Dg d0 d1 d2)))) 0 0 = (slotC ((d2 - d1) • (star a * b))) 0 0 := by
...
-- FULL matrix via full simp with algebra lemmas as simp args + abel fallback.
set_option maxHeartbeats 1600000 in
theorem innerMul_slot_diag_E (a b : O ℚ) (d0 d1 d2 : ℚ) :
    (jb (slotA a) (jb (slotB b) (Dg d0 d1 d2))
  ...
  fin_cases i <;> fin_cases j <;>
    (simp only [jb, slotA, slotB, slotC, Dg, Xz, Matrix.sub_apply, Matrix.add_apply,
       ...
       Matrix.empty_val', Matrix.cons_val_fin_one, Matrix.head_fin_const, Matrix.diagonal_apply,
       Fin.isValue, reduceIte] <;>
     simp only [ocRmulL, ocRmulR, mul_zero, zero_mul, add_zero, zero_add, sub_zero, star_zero,
       sub_self, smul_zero, mul_add, add_mul, smul_add, smul_smul, mul_smul_comm, smul_mul_assoc,
       sub_smul] <;>
     first | rfl | abel | (rw [star_smul]; abel) | simp [star_smul])
