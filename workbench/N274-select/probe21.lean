...
-- WINNING CLOSER: structural simp only; then per entry
--   live entries -> simp only[algebra]; abel (probe14-confirmed, order-independent);
--   zero entries -> simp (probe zero00-confirmed, only full simp crushes octonion 0-products).
set_option maxHeartbeats 1600000 in
theorem innerMul_slot_diag_G (a b : O ℚ) (d0 d1 d2 : ℚ) :
