...
-- UNIFORM CLOSER: one simp only (normalizes all central-scalar occurrences order-independently)
-- then abel closes the module identity in the single atom (star a * b).
...
theorem full_test (a b : O ℚ) (d0 d1 d2 : ℚ) :
  ...
  fin_cases i <;> fin_cases j <;>
    simp only [jb, slotA, slotB, slotC, Dg, Xz, Matrix.sub_apply, Matrix.add_apply,
      ...
      Fin.isValue, reduceIte, mul_zero, zero_mul, add_zero, zero_add, sub_zero, mul_one, one_mul,
      star_zero, sub_self,
