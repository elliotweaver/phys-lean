...
-- SEQUENCED closer: structural -> ocRmul rewrite -> crush zeros -> distribute -> abel.
-- Separate simp only calls control ORDER so zero-crush precedes distribution.
set_option maxHeartbeats 1600000 in
theorem innerMul_slot_diag_D (a b : O ℚ) (d0 d1 d2 : ℚ) :
  ...
  fin_cases i <;> fin_cases j <;>
    (simp only [jb, slotA, slotB, slotC, Dg, Xz, Matrix.sub_apply, Matrix.add_apply,
     ...
     simp only [mul_zero, zero_mul, add_zero, zero_add, sub_zero, star_zero, sub_self, smul_zero] <;>
     first
       | rfl
       | (simp only [smul_smul, mul_add, add_mul, smul_add, mul_smul_comm, smul_mul_assoc,
            sub_smul]; abel))
