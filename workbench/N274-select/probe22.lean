...
-- Test: structural simp only, then `simp [ocRmulL, ocRmulR]` (full simp WITH the scalar lemmas)
-- as a single uniform closer for ALL entries, abel as last resort.
set_option maxHeartbeats 1600000 in
theorem innerMul_slot_diag_H (a b : O ℚ) (d0 d1 d2 : ℚ) :
  ...
  fin_cases i <;> fin_cases j <;>
    simp only [jb, slotA, slotB, slotC, Dg, Xz, Matrix.sub_apply, Matrix.add_apply,
      ...
      Fin.isValue, reduceIte, ocRmulL, ocRmulR, mul_zero, zero_mul, add_zero, zero_add, sub_zero,
      star_zero, smul_zero, mul_add, add_mul, smul_add, sub_smul, smul_smul] <;>
    first | rfl | abel | (rw [star_smul]; abel)
