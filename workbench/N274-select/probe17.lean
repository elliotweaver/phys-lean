...
-- SPLIT CLOSER: structural simp (NO distribution) crushes ifs; then
--   zero entries -> full `simp` (crushes zero-products to 0);
--   live entries -> normalize distribution then abel.
set_option maxHeartbeats 1600000 in
theorem innerMul_slot_diag_C (a b : O ℚ) (d0 d1 d2 : ℚ) :
  ...
  fin_cases i <;> fin_cases j <;>
    ...
    first
      | (simp only [ocRmulL, ocRmulR, mul_add, add_mul, qsmul_mul_left, qsmul_mul_right, sub_smul,
           mul_zero, zero_mul, add_zero, zero_add, sub_zero, star_zero, sub_self]; abel)
      | simp
