/-
  Counterexamples.CosmoStructureGrowingModeVacuityCostume — N455 anti-vacuity (C480).
  ============================================================================================
  W8 ANTI-VACUITY. The N455 node (the gravitational growing mode / the instability of the smooth
  ...
  It is TIED to the banked N455 landing by `iFlag_forced`: the banked `growingMode_concrete_gt`
  witnesses the strict inequality, so the flag is `1`.

  We anchor `min 480 iFlag = 1` (TRUE — `iFlag = 1 < 480`, so the min is `iFlag`).

  THE BOGUS CLAIM (a stand-in for ANY "the mode does not grow / the smooth state is stable / no
  structure forms" mis-reading): that `min 480 iFlag = 480`. It GENUINELY equals `1`
  (`cert_val_true`). Rewriting reduces the bogus claim to the false numeric `1 = 480` in ℕ. The
...
/-- TRUE (tied to the banked N455 landing): the banked `growingMode_concrete_gt` proves
...
/-- TRUE: `min 480 iFlag = 1`, holding precisely because `iFlag = 1 < 480`. -/
theorem cert_val_true : min (480 : ℕ) iFlag = 1 := by decide

/-- BOGUS: claims `min 480 iFlag = 480`. It GENUINELY equals `1` (`cert_val_true`). A "the mode does
    not grow / the smooth state is stable / no structure forms" mis-reading reduces — through the
    banked value — to the false numeric `1 = 480`, so this must NOT compile. -/
theorem cert_val_wrong_BOGUS :
    min (480 : ℕ) iFlag = 480 := by
  rw [cert_val_true]
