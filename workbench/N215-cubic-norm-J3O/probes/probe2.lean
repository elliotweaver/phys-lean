...
/-- ★★ WELL-DEFINEDNESS (cyclic): the trilinear cross-term is CYCLIC in its three octonion
    factors — `reQ ((a*c)*star b) = reQ ((c * star b) * a) = reQ ((star b * a) * c)`. This is
    the structural reason the reduced determinant of a Hermitian matrix over a NONassociative
    ring is well-defined independent of which vertex the cofactor expansion starts at: the
    real part is trace-associative (reQ_mul_assoc3) AND trace-commutative (reQ_mul_comm). The
    SAME one-cause that made the calibration 3-form cyclic, failing beyond the octonions. -/
...
/-- W8 witness 2: the PURE trilinear cross-term. -/
theorem jN_witness_cross : jN 0 0 0 1 1 1 = 2 := by
  unfold jN gForm reQ; simp

#print axioms jN_diag
#print axioms jN_cross_assoc
#print axioms jN_cross_cyclic1
#print axioms jN_cross_cyclic2
#print axioms jN_one
