/- PROBE 71b (W9.7 EXECUTE): two-pass close of (0,1) — distribute, THEN pull ocR central
   (no mul_add in the pull pass, so no loop on nested ocR). (0,1) has deg1=0 ⇒ ocR terms
   cancel additively; residue = bb1_e01. -/
...
-- pull-out levers (banked-shape): ocR central+nuclear ⇒ ocR floats to outermost-left.
...
local macro "distrib" : tactic =>
  `(tactic| simp only [mul_add, add_mul, mul_zero, zero_mul, add_zero, zero_add])
local macro "pullocR" : tactic =>
  `(tactic| simp only [pull_L, pull_M, pull_R1, pull_R2, pull_RR])
local macro "entrysimp0" : tactic =>
  `(tactic| simp only [jdef, jb, Hm, Xz, Fin.isValue, Matrix.mul_apply,
