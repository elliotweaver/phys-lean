/- PROBE (run 78, W9 MEASURE): test closers for polarCross2 (0,0)=0.
   NEW IDEA: ocR_mul collapses ocR·ocR to a single central ocR, so pull-out is single-scalar.
   ocR OPAQUE except via ocR_mul/ocR_comm/ocR_assoc*. KILL 80s. -/
...
-- ocR is a ring hom on the real slot: products collapse.
...
theorem pc2_00 (d0 d1 d2 : ℚ) (a b c p q r : O ℚ) :
  ...
  xzsimp
  -- distribute fully, collapse ocR-products to single ocR, push ocR central, then abel
  simp only [mul_add, add_mul, mul_sub, sub_mul, ocR_mul]
  simp only [← ocR_comm, ocR_assocL, ocR_assocM, ocR_assocR]
  trace_state
  sorry
