/- PROBE (run 78d, W9 MEASURE): close pc2 (0,0) with ocR left-push canonicalizer.
   ocl lemmas via ext (tiny). Bump maxRecDepth. ocR OPAQUE in pc2. KILL 80s. -/
...
-- LEFT-PUSH: every monomial with one ocR collapses to  ocR r * (octonion product).
theorem ocl1 (r : ℚ) (x : O ℚ) : x * ocR r = ocR r * x := by ocrext
...
-- MERGE front-nested ocRs (kills the ocl2 swap loop; strictly reduces ocR-count):
...
theorem pc2_00 (d0 d1 d2 : ℚ) (a b c p q r : O ℚ) :
  ...
  xzsimp
  simp only [mul_add, add_mul, ocR_mul, ocm, ocl1, ocl2, ocl3, ocl4]
  abel
