/- PROBE 79G: CLOSE pc2 (0,1) — OUTWARD ocR lift to front-left + merge, then abel.
   Lift rules (each valid by ocR central+nuclear; each strictly moves ocR toward root-left):
     ocLL (ocR r*x)*y=ocR r*(x*y); ocRL (x*ocR r)*y=ocR r*(x*y);
     ocLR x*(ocR r*y)=ocR r*(x*y); ocRR x*(y*ocR r)=ocR r*(x*y);
     ocMM ocR r*(ocR s*y)=ocR(r*s)*y; ocR_mul merges front product.
   After lifting every monomial = ocR(rational) * (octonion 2-product, NO ocR). abel. KILL 90s. -/
...
local macro "ocrext" : tactic =>
  `(tactic| ext <;> simp only [ocR, mul_re, mul_im, star_re, star_im, add_re, add_im, neg_re,
    neg_im, star_neg, star_star, neg_mul, mul_neg, neg_neg, zero_mul, mul_zero, Dbl.mul_re,
    Dbl.mul_im, Dbl.star_re', Dbl.star_im', Dbl.zero_re, Dbl.zero_im, Dbl.add_re, Dbl.add_im,
    Dbl.neg_re, Dbl.neg_im] <;> ring)

theorem ocR_mul (r s : ℚ) : ocR r * ocR s = ocR (r * s) := by ocrext
theorem ocLL (r : ℚ) (x y : O ℚ) : (ocR r * x) * y = ocR r * (x * y) := by ocrext
theorem ocRL (r : ℚ) (x y : O ℚ) : (x * ocR r) * y = ocR r * (x * y) := by ocrext
theorem ocLR (r : ℚ) (x y : O ℚ) : x * (ocR r * y) = ocR r * (x * y) := by ocrext
theorem ocRR (r : ℚ) (x y : O ℚ) : x * (y * ocR r) = ocR r * (x * y) := by ocrext
theorem ocMM (r s : ℚ) (y : O ℚ) : ocR r * (ocR s * y) = ocR (r * s) * y := by
...
theorem pc2_01 (d0 d1 d2 : ℚ) (a b c p q r : O ℚ) :
  ...
  simp only [ocLL, ocRL, ocLR, ocRR, ocMM, ocR_mul]
  trace_state
  abel
