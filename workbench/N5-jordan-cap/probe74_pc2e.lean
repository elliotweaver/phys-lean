/- PROBE 74: pc2 entry (0,0) and (0,1) with ocR OPAQUE. Dg unfolded to diagonal; off-diag
   Dg entries -> 0, diagonal -> ocR d_k (opaque). pc2 cancels by centrality+nuclearity ALONE
   (free-model NF size 0), so ocR_comm/assoc + abel should close it WITHOUT alternativity. -/
import Phys.Algebra.HermitianJordan.PolarFirst
import Mathlib.Tactic

namespace Probe74PC2e
...
theorem pc2_00 (d0 d1 d2 : ℚ) (a b c p q r : O ℚ) :
  ...
  entrysimp
  simp only [← ocR] -- no-op guard; keep ocR opaque
