/- PROBE 87: can pc1 (the alternativity kernel) close by the COORDINATE route directly?
   After dgsimp, each entry is an octonion polynomial in a,b,c,p,q,r with EXPLICIT ℚ-scalar
   (ocR d_k) coefficients — no opaque ocR (d_k is a rational, expands fine). So cdsimp0 + ring
   may close it (ring knows the full octonion table; alternativity is automatic).
   MEASURE pc1 (0,0) diag and pc1 (0,1) off-diag. KILL 110s. -/
