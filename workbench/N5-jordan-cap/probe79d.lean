/- PROBE 79D: drive pc2 (0,1) entry to a central-scalar polynomial and try to close.
   Strategy: expand every jb...0 1 via Matrix.mul/add entry rules + Dg diagonal entries (0 off
   diag, ocR d_i on diag) + Xz entries; the Dg-diagonal kills most sum terms. Then push ocR
   central and try ring-style close. MEASURE ONLY (timing/leftover). KILL 85s. -/
