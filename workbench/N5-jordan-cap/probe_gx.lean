/- PROBE (run 65, W9.2): measure G_X off-diagonal cost. G_X = X·(X·X) − (X·X)·X, X = Xz a b c
   zero-diagonal Hermitian (octonion-only, NO ocR). FINDINGS: off-diag = 0, ~6 monomials/entry,
   ka-scale alt-burden. Test the (0,1) entry via the bb-style coordinate route. KILL 90s/3.5GB. -/
...
-- G_X (0,1) entry = 0?  G_X = Xz*(Xz*Xz) - (Xz*Xz)*Xz.
theorem gx_e01 (a b c : CD (CD B)) :
    ((Xz a b c) * ((Xz a b c) * (Xz a b c)) - ((Xz a b c) * (Xz a b c)) * (Xz a b c)) 0 1 = 0 := by
  unfold Xz; entrysimp; ext <;> cdsimp0 <;> ring
