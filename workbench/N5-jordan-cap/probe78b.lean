/- PROBE (run 78, W9 MEASURE): print pc2 (0,0) goal AFTER ocR_mul collapse. ocR OPAQUE. -/
...
theorem ocR_mul (r s : ℚ) : ocR r * ocR s = ocR (r * s) := by
  ext <;>
    simp only [ocR, mul_re, mul_im, star_re, star_im, add_re, add_im, neg_re, neg_im,
      star_neg, star_star, neg_mul, mul_neg, neg_neg,
      zero_mul, mul_zero,
...
theorem pc2_00 (d0 d1 d2 : ℚ) (a b c p q r : O ℚ) :
  ...
  xzsimp
  simp only [mul_add, add_mul, ocR_mul]
  trace_state
  sorry
