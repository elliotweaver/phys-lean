/- PROBE (run 56): capture the 3-generator zero-diagonal CORE goal-states for the
   diagonal entry (0,0) and off-diagonal entry (0,1), octonion level. Read-only. -/
...
set_option maxHeartbeats 1000000 in
theorem core01 (a b c p q r : CD (CD B)) : (jdef (Xz a b c) (Xz p q r)) 0 1 = 0 := by
  unfold jdef jb Xz
  entrysimp
  trace_state
  sorry

end ProbeCoreEntries
