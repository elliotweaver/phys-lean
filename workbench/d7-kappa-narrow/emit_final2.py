...
Lmb =(F(-408930664000672,10**14),F(-408930664000670,10**14))   # widened lower by 2 units
base=F(2425,18)
def combo(coeffs):
    lo=hi=F(0)
...
for nm,coeffs,r3lo,r3hi,cmp in [
  ...
  ("DRESSED",(F(3),F(7,2),F(11,2),F(-5,2)),F(1370996710997162,10**13),F(1370996710997180,10**13),">137"),
    ...
    ELO=F(int(mfloor(to_mpf(lo_val)*NUM)),NUM); EHI=F(int(mceil(to_mpf(hi_val)*NUM)),NUM)
    core_lo=klo*CLO>=ELO-base; core_hi=khi*CHI<=EHI-base
    w=EHI-ELO; sig=-log10(float(w)/136.307)
    print(f"=== {nm} ===")
    print(f"  combo exact [{float(clo):.14f},{float(chi):.14f}] w={float(chi-clo):.2e}")
    print(f"  CLO={CLO.numerator}/{CLO.denominator}  CHI={CHI.numerator}/{CHI.denominator}  CLO<=combo? {CLO<=clo} combo<=CHI? {chi<=CHI}")
    ...
    print(f"  narrower than Rung3 ({float(r3hi-r3lo):.2e})? {w<r3hi-r3lo}")
    print()
