...
-- THE COLOUR-MEMBERSHIP of a combination ⟺ b = c = 0
theorem comb_mem_colour_iff (a b c : ℚ) :
    ((a • DI + b • DJ + c • DK : derivationLieQ) : Module.End ℚ (O ℚ)) u1 = 0 ↔ b = 0 ∧ c = 0 := by
  rw [comb_u1]
  constructor
  · intro h
    have h0 : ((-(2:ℚ)*b) • hK + ((2:ℚ)*c) • hJ) = 0 := by
      have := iotaO_injective (a₁ := (-(2:ℚ)*b) • hK + ((2:ℚ)*c) • hJ) (a₂ := 0)
        (by rw [h]; ext <;> simp [iotaO])
      exact this
    obtain ⟨h1, h2⟩ := hJK_indep _ _ h0
    constructor
    · have : -(2:ℚ)*b = 0 := h1; linarith
    · have : (2:ℚ)*c = 0 := h2; linarith
  · rintro ⟨rfl, rfl⟩
    simp only [mul_zero, neg_zero, zero_mul, zero_smul, add_zero]
    ext <;> simp [iotaO]

#print axioms comb_mem_colour_iff
