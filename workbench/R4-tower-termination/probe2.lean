...
-- generic well-founded collapse tool (axiom-free helper)
...
-- ℕ instantiation (axiom-free helper), the concrete well-founded order
...
-- no completed self-representation: the look-back has no live fixed point
...
-- ★ THE WELD: on every LIVE state, the tower terminates by COVER, not by fixed point.
-- (i) no completed self-representation (look p ≠ p); (ii) yet J covers the look-back
-- (J(J p) = look p); (iii) J itself fixes nothing live. One object (J) read twice.
...
#print axioms termination_by_cover_not_fixed_point
#print axioms tower_termination
#print axioms fibre_neg_isFold
#print axioms termination_by_cover_bites
