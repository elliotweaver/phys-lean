...
if 'max_retries' in cols:
    cur.execute("UPDATE tasks SET max_retries=25 WHERE id=?", ('t_f4dbd5f4',))
    con.commit()
    row = cur.execute("SELECT id, max_retries FROM tasks WHERE id=?", ('t_f4dbd5f4',)).fetchone()
    print('N207 after update:', row)
con.close()
