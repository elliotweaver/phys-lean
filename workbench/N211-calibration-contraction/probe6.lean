...
-- PROBE 6: use have + rw for the gForm-to-reQ conversions
...
theorem coassoc4_contraction_probe {x y z w : O ℚ}
  ...
  -- Convert gForm (octCross x y) (octCross z w) = -reQ (octCross x y * octCross z w)
  ...
  have hgx : star (octCross x y) = -(octCross x y) := (mem_ImO).mp hocx
  ...
  rw [h1, h2, h3, h4, h5]
  -- Now everything is in reQ. Expand octCross and assoc.
