...
-- PROBE 14: unfold, then rewrite star(y), star(z), star(w) (star(x) not present after unfold gForm).
-- Then break apart composite stars, then coordinate simp + ring.
...
theorem coassoc4_contraction_probe {x y z w : O ℚ}
  ...
  -- star w, star z, star y appear directly; star x does NOT appear (gForm was unfolded)
  -- But star(z*w), star(z*w + ...) need to be broken apart first
  simp only [star_add, star_mul, star_one, star_zero, neg_neg,
    mul_neg, neg_mul, mul_one, one_mul, add_zero, zero_add, neg_zero]
  -- Now star w, star z, star y should appear
  rw [hsy, hsz, hsw]
