...
theorem coassoc4_contraction {x y z w : O ℚ}
  ...
  -- Unfold coassoc4, octCross, assoc first (these are definitions)
  unfold coassoc4 octCross assoc
  -- Now unfold ALL gForm occurrences using the definitional equation
  simp only [gForm]
  -- Now break all composite stars
