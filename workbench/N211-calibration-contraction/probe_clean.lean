...
-- PROBE: the contraction identity via full coordinate expansion.
-- For imaginary x,y,z,w: coassoc4 x y z w = 2*gForm(octCross x y)(octCross z w) - 2*(gForm x z*gForm y w - gForm x w*gForm y z)
...
theorem coassoc4_contraction_probe {x y z w : O ℚ}
  ...
  -- Break apart composite stars first, then rewrite the individual ones
