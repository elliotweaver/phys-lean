...
-- PROBE 3: break apart composite stars, then substitute, then ring
...
theorem coassoc4_contraction_probe {x y z w : O ℚ}
  ...
  simp only [star_add, star_smul, star_mul, star_one, star_zero, neg_neg, mul_neg, neg_mul,
    mul_one, one_mul, add_zero, zero_add, neg_zero]
  -- Now substitute star x = -x etc
  simp only [hsx, hsy, hsz, hsw]
