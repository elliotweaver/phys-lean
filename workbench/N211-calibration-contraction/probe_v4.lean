...
-- PROBE v4: unfold everything, break stars, rewrite, then coordinate ring
-- The issue is star y appears inside composite terms. Let's use simp with the
-- hypotheses as rewrite rules instead of rw.
...
theorem coassoc4_contraction_probe {x y z w : O ℚ}
  ...
  -- Break apart all composite stars, then simp with the hypotheses
  simp only [star_add, star_mul, star_one, star_smul, hsy, hsz, hsw,
    neg_neg, mul_neg, neg_mul, add_zero, zero_add]
  -- Now coordinate ring
