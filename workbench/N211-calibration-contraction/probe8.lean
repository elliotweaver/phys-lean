...
-- PROBE 8: follow N210's exact simp set, unfold all before simp
...
theorem coassoc4_contraction_probe {x y z w : O ℚ}
  ...
  -- First convert LHS using the bridge
  rw [coassoc4_eq_neg_reQ hw]
  -- Now convert ALL gForm on RHS to reQ (imaginary star = neg)
  ...
  rw [h1, h2, h3, h4, h5]
  -- Now unfold octCross and assoc (NO star left to cause loops)
  unfold octCross assoc
  -- At this point there should be NO star calls remaining.
  -- The only thing left is reQ of products.
  -- Use the N210 simp set (but star lemmas should be no-ops now)
