...
-- PROBE v10: The simp works but ring fails (expression too large? or non-commutative residue?)
-- Let me try with noncomm_ring instead of ring, or try to simplify first.
...
theorem coassoc4_contraction_probe {x y z w : O ℚ}
  ...
  -- The coordinate expansion is in ℚ (commutative). Try ring with higher heartbeats.
  ring
