...
if 'max_retries' in cols:
    cur.execute("UPDATE tasks SET max_retries=25 WHERE id='t_b307d7bb'")
    con.commit()
    print('set max_retries=25:', cur.execute("SELECT id, status, max_retries FROM tasks WHERE id='t_b307d7bb'").fetchone())
else:
    print('cols:', cols)
