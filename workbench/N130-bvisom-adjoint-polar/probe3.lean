...
/-! ## W8 witnesses — a concrete boost is a genuine BvC-isometry whose converse decomposition
    genuinely returns it. -/

-- the boost (5/3,4/3) is a genuine BvC-isometry (off-diagonal 4/3 ≠ 0 — it MOVES the timelike axis)
-- its converse polar reconstruction returns the boost: read space coordinate 4/3 at (1,0,0).
theorem probe_bvIsomLin_boost_recon_coord :
    ∃ (h : Module.End Cut STVC) (n : ℕ) (c : Fin n → Cut) (u : Fin n → STVC)
      (γ : Cut → Module.End Cut STVC),
  ...
  obtain ⟨h, n, c, u, γ, h1, _, _, _, _, _, _, h8⟩ :=
    probe_bvIsomLin_polar_path_decomp (boostEndC_isBvIsomLin hab)
  refine ⟨h, n, c, u, γ, h1, ?_⟩
  rw [h8, boostEndC_apply]
  norm_num

-- the converse decomposition preserves the indefinite self-overlap: reading 1 at the moved axis.
theorem probe_bvIsomLin_boost_witness_self_overlap :
    ∃ (h : Module.End Cut STVC) (n : ℕ) (c : Fin n → Cut) (u : Fin n → STVC)
      (γ : Cut → Module.End Cut STVC),
      h.comp (boostEndC ((5:Cut)/3) ((4:Cut)/3)) = specOpN c u ∧
