...
theorem restricted_equivariance {T : Module.End ℚ (Matrix (Fin 3) (Fin 3) (O ℚ))}
  ...
  -- Evaluate the RHS bracket at X. Rewrite the bracket to a difference of composites first.
  have hbr : (⁅T.restrict (derH3_mapsTo hT), (Lmul A).restrict (lmul_mapsTo hA)⁆
      : Module.End ℚ hermSub)
      = T.restrict (derH3_mapsTo hT) * (Lmul A).restrict (lmul_mapsTo hA)
        - (Lmul A).restrict (lmul_mapsTo hA) * T.restrict (derH3_mapsTo hT) := Ring.lie_def _ _
  rw [hbr]
  simp only [LinearMap.sub_apply, Module.End.mul_apply, Submodule.coe_sub,
...
theorem derH3_kills_hermTrace {T : Module.End ℚ (Matrix (Fin 3) (Fin 3) (O ℚ))}
  ...
  rw [restricted_equivariance hT hA]; exact LinearMap.trace_lie _ _
...
theorem dilation_hermTrace_ne :
  ...
  have h := finrank_hermSub_pos
  have : (0:ℚ) < (Module.finrank ℚ hermSub : ℚ) := by exact_mod_cast h
  positivity
