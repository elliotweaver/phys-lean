...
# ==== N294 probe2: de-risk the colour<->flavour route ====
flav=centralizer_in_f4(colour_lift)
FL=np.array([m.reshape(-1) for m in flav])
COSET3=[kAB,kBC,kCA]

def lie_generated(gens, ambientFlat, maxiter=8, tol=1e-7):
    """Dim of Lie algebra generated by gens (list of NxN), living in ambient."""
    cur=[g.copy() for g in gens]
    def spanrank(mats): return np.linalg.matrix_rank(np.array([m.reshape(-1) for m in mats]),tol=tol)
    r=spanrank(cur)
    for _ in range(maxiter):
        new=list(cur)
        for a in range(len(cur)):
            for b in range(a+1,len(cur)):
                new.append(comm(cur[a],cur[b]))
        # reduce to a basis
        M=np.array([m.reshape(-1) for m in new])
        _,s,v=np.linalg.svd(M); rr=int(np.sum(s>tol))
        # orthonormal basis reshaped
        cur=[v[k].reshape(N,N) for k in range(rr)]
        nr=len(cur)
        if nr==r: break
        r=nr
    return r,cur

print("\n=== Q1: does the fold-root-dressed coset {kAB,kBC,kCA} Lie-generate flavour su(3)? ===")
dimgen,genbasis=lie_generated(COSET3, FL)
print("dim Lie<{kAB,kBC,kCA}> =",dimgen,"(want 8 = flavour su(3))")
GEN=np.array([m.reshape(-1) for m in genbasis])
print("generated == flavour su(3)? rank(gen U flav) =",rankof(np.vstack([GEN,FL])),"(==8 => equal)")

print("\n=== Q2: colour commutes with ALL of flavour su(3)? (whole-algebra colour-neutrality) ===")
maxres=0.0
for D in colour_lift:
    for fm in flav:
        maxres=max(maxres, np.max(np.abs(comm(D,fm))))
print("max |[jActL(colour), flavour-basis]| =",maxres,"(0 => colour centralizes ALL flavour)")
# and specifically the coset gens
for nm,k in [("kAB",kAB),("kBC",kBC),("kCA",kCA)]:
    m=max(np.max(np.abs(comm(D,k))) for D in colour_lift)
    print(f"  max |[colour,{nm}]| = {m:.2e}")

print("\n=== Q3: disjointness witness — flavour generator OUTSIDE jActL(g2)? ===")
# is kAB in jActL(g2)? (gauge entrywise)
print("kAB in jActL(g2)? res =",proj_res(kAB,GA),"(nonzero => kAB escapes gauge)")
print("famC in jActL(g2)? res =",proj_res(famC,GA),"(nonzero => family escapes gauge too)")
print("rank(jActL(g2) U flavour) =",rankof(np.vstack([GA,FL])),"(==14+8=22 => fully disjoint)")
print("rank(jActL(colour) U flavour) =",rankof(np.vstack([CA,FL])),"(==16 => colour+flavour disjoint)")

print("\n=== Q4: FULL mutual centralizer Z_{f4}(flavour) = colour? ===")
Zflav=centralizer_in_f4(flav)
ZFL=np.array([m.reshape(-1) for m in Zflav])
print("dim Z_{f4}(flavour su(3)) =",len(Zflav))
print("rank(Z_{f4}(flavour) U jActL(colour)) =",rankof(np.vstack([ZFL,CA])),"(==8 => equals colour EXACTLY)")
print("each colour in Z_{f4}(flavour)? maxres=",max(proj_res(m,ZFL) for m in colour_lift))
print("\nDONE probe2")
