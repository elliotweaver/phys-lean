...
/-- `x * star x` is self-conjugate — NO coordinates: star anti-mult + involutive. -/
theorem selfmul_selfconj (x : O ℚ) : star (x * star x) = x * star x := by
  rw [star_mul, star_star]

/-- ★ THE NORM-PRESERVATION IDENTITY (`Der ⊆ so(form)`). A derivation kills the
    self-overlap `x * star x` (it is self-conjugate), so by Leibniz + `deriv_star`:
    `D x * star x = x * D x`. This is skew-adjointness wrt the Born self-overlap. -/
theorem deriv_norm_preserve (D : Module.End ℤ (O ℚ)) (hD : IsDeriv D) (x : O ℚ) :
  ...
  rw [hD x (star x), deriv_star D hD x] at hzero
  -- hzero : D x * star x + x * (- D x) = 0
  rw [mul_neg] at hzero
