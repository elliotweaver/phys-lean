...
-- G2: finrank ℚ V = n * finrank ℚ ↥S, via restrictScalars of the A-linear equiv.
example (A : Subalgebra ℚ (Module.End ℚ V)) (n : ℕ) (S : Submodule A V)
    (e : V ≃ₗ[A] (Fin n → S)) :
    Module.finrank ℚ V = n * Module.finrank ℚ S := by
  haveI : FiniteDimensional ℚ S := by
    have h : FiniteDimensional ℚ (S.restrictScalars ℚ) := inferInstance
    exact h
