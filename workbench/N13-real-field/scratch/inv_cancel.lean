...
/-! The inverse cut (recovered from inv_gauge.lean). -/
...
theorem mul_approx {x : Cut} (hx0 : (0:Cut) < x) {t : Q} (ht0 : 0 < t) (ht1 : t < 1) :
  ...
  -- t*r < A  ⟺  r - A < (1-t)*r ... use r - A < ε = a0(1-t) ≤ A(1-t) ≤ r(1-t)? need t*r<A
  -- t*r < A ⟺ A - t*r > 0. We have A - t*A = A(1-t) ≥ a0(1-t) = ε > r - A.
  -- so A - t*A > r - A ⟹ 2A - t*A > r ⟹ ... not directly. Use: A(1-t) ≥ ε > r-A ⟹
  -- A - tA > r - A; want A - t r > 0. Since r > A, t r > t A, so A - t r < A - t A. WRONG dir.
  -- Correct: want t*r < A. We have r < A + ε ≤ A + A(1-t) = A(2-t). Then
  -- t*r < t*A*(2-t) = A*(2t - t²) = A*(1-(1-t)²) < A. ✓ (since (1-t)²>0)
