...
/-!
  # SCRATCH N675 — MirrorTermLanguage: the moment pair at the next scales + the universal
  # legality theorem over the formal term language (the N588 boundary closed)
  PREREG: workbench/mirror-closure/PREREG-AIRTIGHT.md (N675). Recon: airtight-recon.json
  (cos⁵·sin = 5/32·sin2x + 1/8·sin4x + 1/32·sin6x — pure addition-formula identity;
   cos⁴·sin = 1/8·sin x + 3/16·sin 3x + 1/16·sin 5x — one Pythagorean factor,
   certificate c²s − s³ + s; both verified exact by sympy pre-Lean).
-/
