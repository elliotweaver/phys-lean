...
theorem e2amp (M A p r : ℚ) (h : 3*p^2+r^2=3) (hA : A^2=2*M^2) :
  ...
  have hsq : (genVec M A p r 0 + genVec M A p r 1 + genVec M A p r 2)^2 = 9*M^2 := by rw [h1]; ring
  nlinarith [hsq, hs]

-- σ2(m) = e2(a)² − 2·e1(a)·e3(a) — the ring identity, THEN substitute forced values.
theorem sigma2m (M A p r : ℚ) (h : 3*p^2+r^2=3) (hA : A^2=2*M^2) :
  ...
  have key : massVecP M A p r 0 * massVecP M A p r 1 + massVecP M A p r 0 * massVecP M A p r 2
      + massVecP M A p r 1 * massVecP M A p r 2
    = (genVec M A p r 0 * genVec M A p r 1 + genVec M A p r 0 * genVec M A p r 2
        + genVec M A p r 1 * genVec M A p r 2)^2
      - 2*(e3ampP M A p r)*(genVec M A p r 0 + genVec M A p r 1 + genVec M A p r 2) := by
...
theorem deg_delta0 (M A : ℚ) : genVec M A 1 0 1 = genVec M A 1 0 2 := by simp [genVec]
theorem distinct01 (M A : ℚ) (hA : A ≠ 0) :
...
theorem distinct12 (M A : ℚ) (hA : A ≠ 0) :
  ...
  intro hc; apply hA; linarith
theorem distinct02 (M A : ℚ) (hA : A ≠ 0) :
    genVec M A (1/7) (-12/7) 0 ≠ genVec M A (1/7) (-12/7) 2 := by
  simp only [genVec, Matrix.cons_val_zero, Matrix.cons_val_two, Matrix.tail_cons, Matrix.head_cons]
  intro hc; apply hA; linarith
