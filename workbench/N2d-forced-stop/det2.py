...
def randoct():
    v=z0(L)
    for i in range(8): v=radd(v, scale(Fr(random.randint(-1,1)),basis(L,i),L),L)
    return v
random.seed(7)
trips=[(randoct(),randoct(),randoct()) for _ in range(40)]
keys=['0','u','v','w','U','V','W']
def comps(u,v,w):
...
for fa in itertools.product(keys,repeat=4):
    ...
    for (u,v,w) in trips:
        ...
        if not dz: bc+=1
    if ok and bc>0: best.append((nz,bc,fa))
best.sort(key=lambda t:(t[0],-t[1]))
print("biconditional witnesses:",len(best))
for nz,bc,fa in best[:10]: print("  nz",nz,"bites",bc,fa)
