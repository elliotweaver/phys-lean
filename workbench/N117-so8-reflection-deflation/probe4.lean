import Phys.Algebra.LorentzContinuumGenerationDeflationStep

namespace Phys.Algebra

open Phys.Cascade Phys.Cascade.CD
open Phys.Foundation Phys.Foundation.ContinuumQ

attribute [local instance] CD.narCD CD.srCD
attribute [local instance] Phys.Algebra.dblModuleCut

noncomputable section
open scoped BigOperators

theorem trace_id_cut (z : O Cut) : z + star z = (2 * reQC z) • (1 : O Cut) := by
  ext <;> simp [reQC] <;> ring

theorem gFormC_imag_right {q : O Cut} (hq : star q = -q) (p : O Cut) :
    gFormC p q = - reQC (p * q) := by
  unfold gFormC; rw [hq, mul_neg p q, reQC_neg]

theorem polarize_imag {p q : O Cut} (hp : star p = -p) (hq : star q = -q) :
    p * q + q * p = (-(2 * gFormC p q)) • (1 : O Cut) := by
  have ht : (p * q) + star (p * q) = (2 * reQC (p * q)) • (1 : O Cut) := trace_id_cut (p * q)
  rw [star_mul, hp, hq, neg_mul_neg] at ht
  rw [gFormC_imag_right hq p]
  rw [show (-(2 * - reQC (p * q))) = 2 * reQC (p * q) by ring]
  exact ht

theorem reflect_imag {m : O Cut} (hm : star m = -m) (hL : ∀ z : O Cut, m * (m * z) = -z)
    {v : O Cut} (hv : star v = -v) :
    m * (v * m) = v - (2 * gFormC v m) • m := by
  have hpol := polarize_imag hv hm
  have hvm : v * m = (-(2 * gFormC v m)) • (1 : O Cut) - m * v := by
    rw [eq_sub_iff_add_eq]; exact hpol
  rw [hvm, mul_sub]
  rw [show m * ((-(2 * gFormC v m)) • (1 : O Cut)) = (-(2 * gFormC v m)) • m by
        rw [smulCompat_oCut.hr, mul_one]]
  rw [hL v]
  rw [sub_neg_eq_add, neg_smul]; abel

-- THE SWAP.  m := e - w (unnormalized), e, w unit-imaginary, gFormC e e = gFormC w w = 1, e ≠ w.
-- We want R_m(w) = w - (2 gFormC w m / gFormC m m) • m to equal e.
-- gFormC m m = gFormC (e-w) (e-w) = gFormC e e - 2 gFormC e w + gFormC w w = 2 - 2 gFormC e w
-- gFormC w m = gFormC w (e-w) = gFormC w e - gFormC w w = gFormC e w - 1
-- So 2 gFormC w m / gFormC m m = 2(gFormC e w - 1)/(2 - 2 gFormC e w) = (gFormC e w -1)/(1 - gFormC e w) = -1
-- Hence R_m(w) = w - (-1)•(e - w) = w + e - w = e.  CLEAN — independent of normalization!
-- But reflect_imag uses m*(v*m) with NORMALIZED m (m*m=-1).  The half-turn biMulV m m for a
-- non-unit m is NOT the reflection; it scales.  Two routes:
--  (A) normalize m to unit, then reflect_imag gives the reflection.
--  (B) use the UNNORMALIZED reflection identity m*(v*m) = (2 gFormC v m)•m - (gFormC m m)•v  [no unit assumption!]
-- Route B is cleaner — derive the general (non-unit) two-sided reflection identity.

-- GENERAL reflection identity (no unit assumption on m), m imaginary, v imaginary:
-- m*(v*m) = (2 gFormC v m)•m - (gFormC m m)•v
-- Derivation: polarize: v*m + m*v = -(2 gFormC v m)•1, so v*m = -(2 gFormC v m)•1 - m*v.
-- m*(v*m) = -(2 gFormC v m)•m - m*(m*v) = -(2 gFormC v m)•m - (m*m)*v  [left-alt: m*(m*v)=(m*m)*v]
-- m*m = ?  For imaginary m: m*m = -(gFormC m m)•1  (since m*star m = (gFormC m m)•1, star m = -m)
-- so (m*m)*v = -(gFormC m m)•v.  Thus m*(v*m) = -(2 gFormC v m)•m + (gFormC m m)•v.
-- Hmm sign: that's (gFormC m m)•v - (2 gFormC v m)•m.  Let me just prove and read.
theorem selfconj_eq_smul_cut {y : O Cut} (h : star y = y) : y = (reQC y) • (1 : O Cut) := by
  have ht := trace_id_cut y
  rw [h] at ht
  -- ht : y + y = (2 * reQC y) • 1
  have h2 : (2 : Cut) • y = (2 * reQC y) • (1 : O Cut) := by rw [two_smul]; exact ht
  have hc := congrArg (fun z => (2⁻¹ : Cut) • z) h2
  simp only [smul_smul] at hc
  rw [show (2⁻¹ : Cut) * 2 = 1 by norm_num, one_smul,
      show (2⁻¹ : Cut) * (2 * reQC y) = reQC y by ring] at hc
  exact hc

theorem mm_imag {m : O Cut} (hm : star m = -m) : m * m = (-(gFormC m m)) • (1 : O Cut) := by
  have hsc : star (m * star m) = m * star m := by
    rw [star_mul, star_star]
  have hreal : m * star m = (reQC (m * star m)) • (1 : O Cut) := selfconj_eq_smul_cut hsc
  have hg : reQC (m * star m) = gFormC m m := rfl
  rw [hg] at hreal
  -- hreal : m * star m = gFormC m m • 1.  star m = -m ⇒ m * (-m) = gFormC m m • 1 ⇒ m*m = -(...)
  rw [hm] at hreal
  rw [mul_neg m m] at hreal
  -- hreal : -(m * m) = gFormC m m • 1
  rw [show m * m = -(-(m * m)) by rw [neg_neg], hreal, neg_smul]

theorem reflect_imag_general {m : O Cut} (hm : star m = -m)
    {v : O Cut} (hv : star v = -v) :
    m * (v * m) = (gFormC m m) • v - (2 * gFormC v m) • m := by
  have hpol := polarize_imag hv hm
  have hvm : v * m = (-(2 * gFormC v m)) • (1 : O Cut) - m * v := by
    rw [eq_sub_iff_add_eq]; exact hpol
  rw [hvm, mul_sub]
  rw [show m * ((-(2 * gFormC v m)) • (1 : O Cut)) = (-(2 * gFormC v m)) • m by
        rw [smulCompat_oCut.hr, mul_one]]
  -- m*(m*v) = (m*m)*v  (left alternative)
  rw [show m * (m * v) = (m * m) * v from (mul_mul_left m v)]
  rw [mm_imag hm]
  rw [show ((-(gFormC m m)) • (1 : O Cut)) * v = (-(gFormC m m)) • v by
        rw [smulCompat_oCut.hl, one_mul]]
  rw [neg_smul, neg_smul, sub_neg_eq_add]; abel

end

end Phys.Algebra
