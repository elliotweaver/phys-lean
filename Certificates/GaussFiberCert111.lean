/-
  Certificates.GaussFiberCert111 — fiber-h=111 EXCLUSION certificate for
  D = −13947439 (first odd-disc fundamental above X₀(111)).
  111 deep touching primes drive fiber_exclusion (N624):
  any complete duplicate-free box of disc −13947439 has size > 111,
  so h(−13947439) ≥ 112 — D is excluded from every fiber ≤ 111.
...
theorem cert : ∀ (L : List GaussForms.BQF) (pf : GaussForms.BQF),
  ...
  apply fiber_exclusion (D := -13947439) (c₀ := 3486860) (ps := ps) (h := 111)
  · norm_num
  · norm_num
  · exact hpw
  · exact hmem
