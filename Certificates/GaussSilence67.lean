/-
  Certificates.GaussSilence67 — THE SILENCE of D = -67: every
  deep odd prime (3) is TOTALLY BLOCKED on the
  principal branch x² + x + 17 — deferral_law from the finite checks.
...
namespace Certificates.Silence67
open Phys.Foundation.GaussForms.BQF
open Phys.Foundation

/-- 3 never divides x² + x + 17 — from 2 checks. -/
theorem blocked_3 : ∀ x : Z, ¬ ZDvd' 3 (x * x + x + 17) := by
  ...
  · -- P(0) = 17: 3 ∤ 17
    have h2 : 3 * (k - 5) = 2 := by linarith
    rcases lt_trichotomy (k - 5) 0 with hc | hc | hc
    · nlinarith [z_pos_ge_one (show (0:Z) < -(k - 5) by linarith)]
    · rw [hc] at h2; norm_num at h2
    · nlinarith [z_pos_ge_one hc]
  · -- P(1) = 19: 3 ∤ 19
    have h2 : 3 * (k - 6) = 1 := by linarith
    rcases lt_trichotomy (k - 6) 0 with hc | hc | hc
    · nlinarith [z_pos_ge_one (show (0:Z) < -(k - 6) by linarith)]
    · rw [hc] at h2; norm_num at h2
    · nlinarith [z_pos_ge_one hc]

/-- THE SILENCE: every deep odd prime of D = -67 is blocked. -/
theorem silence : ∀ p : Z, (p = 3) →
    ∀ x : Z, ¬ ZDvd' p (x * x + x + 17) := by
  intro p hp x
  rcases hp with rfl
  · exact blocked_3 x
