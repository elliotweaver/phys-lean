/-
  Certificates.GaussFiberCert137 — fiber-h=137 EXCLUSION certificate for
  D = −21736707 (first odd-disc fundamental above X₀(137)).
  137 deep touching primes drive fiber_exclusion (N624):
  any complete duplicate-free box of disc −21736707 has size > 137,
  so h(−21736707) ≥ 138 — D is excluded from every fiber ≤ 137.
...
theorem cert : ∀ (L : List GaussForms.BQF) (pf : GaussForms.BQF),
  ...
  apply fiber_exclusion (D := -21736707) (c₀ := 5434177) (ps := ps) (h := 137)
  · norm_num
  · norm_num
  · exact hpw
  · exact hmem
