/-
  Certificates.GaussSilence163 — THE SILENCE of D = -163: every
  deep odd prime (3, 5) is TOTALLY BLOCKED on the
  principal branch x² + x + 41 — deferral_law from the finite checks.
...
namespace Certificates.Silence163
...
/-- 3 never divides x² + x + 41 — from 2 checks. -/
theorem blocked_3 : ∀ x : Z, ¬ ZDvd' 3 (x * x + x + 41) := by
...
/-- 5 never divides x² + x + 41 — from 3 checks. -/
theorem blocked_5 : ∀ x : Z, ¬ ZDvd' 5 (x * x + x + 41) := by
...
/-- THE SILENCE: every deep odd prime of D = -163 is blocked. -/
theorem silence : ∀ p : Z, (p = 3 ∨ p = 5) →
    ∀ x : Z, ¬ ZDvd' p (x * x + x + 41) := by
  intro p hp x
