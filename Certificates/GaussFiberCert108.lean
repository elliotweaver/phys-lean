/-
  Certificates.GaussFiberCert108 — fiber-h=108 EXCLUSION certificate for
  D = −13184571 (first odd-disc fundamental above X₀(108)).
  108 deep touching primes drive fiber_exclusion (N624):
  any complete duplicate-free box of disc −13184571 has size > 108,
  so h(−13184571) ≥ 109 — D is excluded from every fiber ≤ 108.
...
theorem cert : ∀ (L : List GaussForms.BQF) (pf : GaussForms.BQF),
  ...
  apply fiber_exclusion (D := -13184571) (c₀ := 3296143) (ps := ps) (h := 108)
  · norm_num
  · norm_num
  · exact hpw
  · exact hmem
