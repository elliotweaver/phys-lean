/-
  Certificates.GaussFiberCert113 — fiber-h=113 EXCLUSION certificate for
  D = −15483031 (first odd-disc fundamental above X₀(113)).
  113 deep touching primes drive fiber_exclusion (N624):
  any complete duplicate-free box of disc −15483031 has size > 113,
  so h(−15483031) ≥ 114 — D is excluded from every fiber ≤ 113.
...
theorem cert : ∀ (L : List GaussForms.BQF) (pf : GaussForms.BQF),
  ...
  apply fiber_exclusion (D := -15483031) (c₀ := 3870758) (ps := ps) (h := 113)
  · norm_num
  · norm_num
  · exact hpw
  · exact hmem
